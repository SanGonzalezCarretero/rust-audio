            ...
            if !parameters.is_empty() {
                for (param_name, param_value) in parameters {
                    ...
                    let is_editing = app
                        ...
                        .as_ref()
                        .map(|(edit_idx, edit_param)| *edit_idx == idx && edit_param == &param_name)
                        .unwrap_or(false);
                    ...
                    rows.push(ListRow::Parameter(idx, param_name.clone(), param_value.clone()));
                }
            }
        }
    }
        ...
        KeyCode::Enter => {
            ...
            match rows.get(app.selected) {
                Some(ListRow::Effect(effect_idx)) => {
                        ...
                        .find(|(idx, _)| *idx == *effect_idx)
                    {
                        if !params.is_empty() {
                            ...
                            app.active_parameter_edit = Some((*effect_idx, first_param.clone()));
                            app.input_buffer = first_value;
                            app.status = format!("Editing {} (Enter to confirm, Esc to cancel)", first_param);
                        }
                    } else if let Some(effect) = app
                        .selected_effects
                        .iter()
                        .find(|e| e.same_variant(effect_variant))
                    {
                        ...
                        if !parameters.is_empty() {
                            ...
                            app.status = "This effect has no configurable parameters".to_string();
                        }
            } else {
                        app.status = "Enable the effect first to configure parameters".to_string();
                    }
                }
                Some(ListRow::Parameter(effect_idx, param_name, param_value)) => {
                    ...
                    app.status = format!("Editing {} (Enter: confirm, Esc: cancel)", param_name);
                }
                None => {}
            }
        }
